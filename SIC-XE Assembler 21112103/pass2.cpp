...
bool readIntermediateFile(ifstream &readFile, bool &isComment, int &lineNumber, int &address, int &blockNumber, string &label, string &opcode, string &operand, string &comment)
{
  ...
  lineNumber = stringtodec(readTillTab(fileLine, index));
  ...
  {
    blockNumber = stringtodec(tempBuffer);
  }
...
string createObjectCodeFormat34()
{
  ...
  { // Immediate
    ...
    { // Error handling for Immediate with index based
      ...
      objcode = inttostrhe(stringHexToInt(OPTAB[getRealOpcode(opcode)].opcode) + 1, 2);
    ...
    { // Immediate integer value
      ...
      {
        immediateValue = stringtodec(tempOperand);
      }
      else
      {
      ...
      else
      {
        ...
        objcode += inttostrhe(immediateValue, halfBytes);
      }
      return objcode;
    }
    else if (SYMTAB[tempOperand].exists == 'n' || CSECT_TAB[currentSectName].EXTREF_TAB[tempOperand].exists == 'y')
    {
      ...
      if (SYMTAB[tempOperand].exists == 'y' && CSECT_TAB[currentSectName].EXTREF_TAB[tempOperand].exists == 'y')
      {
        ...
        modificationRecord += "M^" + inttostrhe(address + 1, 6) + '^';
    ...
    else
    {
      ...
      { /*If format 4*/
        objcode = inttostrhe(stringHexToInt(OPTAB[getRealOpcode(opcode)].opcode) + 1, 2);
        objcode += '1';
        objcode += inttostrhe(operandAddress, halfBytes);
      ...
      if (relativeAddress >= (-2048) && relativeAddress <= 2047)
      {
        objcode = inttostrhe(stringHexToInt(OPTAB[getRealOpcode(opcode)].opcode) + 1, 2);
        objcode += '2';
        objcode += inttostrhe(relativeAddress, halfBytes);
        return objcode;
      }
      ...
      if (operandAddress <= 4095)
      {
        ...
        /*add modifacation record here*/
        modificationRecord += "M^" + inttostrhe(address + 1 + stringHexToInt(BLOCKS[BLocksNumToName[blockNumber]].startAddress), 6) + '^';
        modificationRecord += (halfBytes == 5) ? "05" : "03";
        modificationRecord += '\n';
  ...
  else if (getFlagFormat(operand) == '@')
  {
    ...
    { // Error handling for Indirect with index based
      ...
      /*****/ } /*****/
      writeToFile(errorFile, writeData);
      objcode = inttostrhe(stringHexToInt(OPTAB[getRealOpcode(opcode)].opcode) + 2, 2);
      objcode += (halfBytes == 5) ? "100000" : "0000";
      return objcode;
      }
      /*
    ...
    writeData = "Line: " + to_string(lineNumber);
    writeData += "Can't fit into program counter based or base register please use format 4 .";
    writeToFile(errorFile, writeData);
    objcode = inttostrhe(stringHexToInt(OPTAB[getRealOpcode(opcode)].opcode) + 2, 2);
    objcode += (halfBytes == 5) ? "100000" : "0000";
  ...
  { // Literals
    ...
    if (tempOperand == "*")
    {
      tempOperand = "X'" + inttostrhe(address, 6) + "'";
      /*Add modification record for value created by operand `*` */
      modificationRecord += "M^" + inttostrhe(stringHexToInt(LITTAB[tempOperand].address) + stringHexToInt(BLOCKS[BLocksNumToName[LITTAB[tempOperand].blockNumber]].startAddress), 6) + '^';
      modificationRecord += inttostrhe(6, 2);
      modificationRecord += '\n';
    }
    ...
    if (LITTAB[tempOperand].exists == 'n')
    {
      ...
      objcode = inttostrhe(stringHexToInt(OPTAB[getRealOpcode(opcode)].opcode) + 3, 2);
    ...
    writeData = "Line: " + to_string(lineNumber);
    writeData += "Can't fit into program counter based or base register based addressing please use format  4.";
    writeToFile(errorFile, writeData);
    objcode = inttostrhe(stringHexToInt(OPTAB[getRealOpcode(opcode)].opcode) + 3, 2);
  ...
  { /*Handle direct addressing*/
    ...
    if (SYMTAB[tempOperand].exists == 'n' /*****/ || CSECT_TAB[currentSectName].EXTREF_TAB[tempOperand].exists == 'y' /*****/)
    {
      ...
      writeToFile(errorFile, writeData);
      objcode = inttostrhe(stringHexToInt(OPTAB[getRealOpcode(opcode)].opcode) + 3, 2);
      objcode += (halfBytes == 5) ? (inttostrhe(xbpe + 1, 1) + "00") : inttostrhe(xbpe, 1);
      objcode += "000";
      return objcode;
      }
...
void writeTextRecord(bool lastRecord = false)
{
  if (lastRecord)
  {
    ...
    { // Write last text record
      writeData = inttostrhe(currentRecord.length() / 2, 2) + '^' + currentRecord;
  ...
  if (objectCode != "")
  {
    if (currentRecord.length() == 0)
    {
      writeData = "T^" + inttostrhe(address + stringHexToInt(BLOCKS[BLocksNumToName[blockNumber]].startAddress), 6) + '^';
...
void writeDRecord()
{
  ...
  for (int i = 0; i < length; i++)
  {
    ...
    write_D_Data += stringexpand(inp, 6, ' ', true) + temp_address;
    inp = "";
  }
...
void writeRRecord()
{
  ...
  for (int i = 0; i < length; i++)
  {
    ...
    write_R_Data += stringexpand(inp, 6, ' ', true);
    inp = "";
  }
...
void writeEndRecord(bool write = true)
{
  if (write)
  {
    ...
    {
      if(currentSectName=="RDREC" && word_det)
        writeToFile(objectFile,det);
  ...
  { // If no operand of END
    endRecord = "E^" + inttostrhe(startAddress, 6);
  }
  ...
  { // Make operand on end firstExecutableAddress
    ...
    endRecord = "E^" + inttostrhe(firstExecutableAddress, 6) + "\n";
  }
}
...
void pass2()
{
  ...
  if (opcode == "START")
  {
    ...
    writeData = to_string(lineNumber) + "\t" + inttostrhe(address) + "\t" + to_string(blockNumber) + "\t" + label + "\t" + opcode + "\t" + operand + "\t" + objectCode + "\t" + comment;
    writeToFile(ListingFile, writeData);
  }
  else
  {
  ...
  writeData = "H^" + stringexpand(label, 6, ' ', true) + '^' + inttostrhe(address, 6) + '^' + inttostrhe(program_section_length, 6);
  writeToFile(objectFile, writeData);
  ...
  while (opcode != "END")
  {
    while (opcode != "END" && opcode != "CSECT")
    {
      ...
      if (OPTAB[getRealOpcode(opcode)].exists == 'y')
      {
        ...
        else if (OPTAB[getRealOpcode(opcode)].format == 2)
        {
          ...
          { // If not two operand i.e. a
            ...
            {
              objectCode = OPTAB[getRealOpcode(opcode)].opcode + inttostrhe(stringtodec(operand1), 1) + '0';
            }
          ...
          { // Two operands i.e. a,b
            ...
            {
              objectCode = OPTAB[getRealOpcode(opcode)].opcode + REGTAB[operand1].num + inttostrhe(stringtodec(operand2), 1);
            }
      ...
      else if (opcode == "BYTE")
      {
        ...
        {
          objectCode = str_to_he_str(operand.substr(2, operand.length() - 3));
        }
      }
      else if (label == "*")
      {
        if (opcode[1] == 'C')
        {
          objectCode = str_to_he_str(opcode.substr(3, opcode.length() - 4));
        }
        else if (opcode[1] == 'X')
        {
      ...
      {
        objectCode = inttostrhe(stringtodec(operand), 6);
      }
      else if (opcode == "BASE")
      {
      ...
      {
        writeData = to_string(lineNumber) + "\t" + inttostrhe(address) + "\t" + " " + "\t" + label + "\t" + opcode + "\t" + operand + "\t" + objectCode + "\t" + comment;
      }
      else if (address == -1)
      {
        if (opcode == "EXTDEF")
    ...
    {
      writeData = to_string(lineNumber) + "\t" + inttostrhe(address) + "\t" + to_string(blockNumber) + "\t" + label + "\t" + opcode + "\t" + " " + "\t" + objectCode + "\t" + " ";
    }
    else if (!isComment)
    {
      writeData = to_string(lineNumber) + "\t" + inttostrhe(address) + "\t" + " " + "\t" + label + "\t" + opcode + "\t" + operand + "\t" + "" + "\t" + comment;
    }
    else
    {
      writeData = to_string(lineNumber) + "\t" + comment;
    ...
    if (!isComment && opcode != "END")
    {
      ...
      writeData = "\nH^" + stringexpand(label, 6, ' ', true) + '^' + inttostrhe(address, 6) + '^' + inttostrhe(CSECT_TAB[label].length, 6);
      writeToFile(objectFile, writeData);
    }
    //********************************
    readIntermediateFile(intermediateFile, isComment, lineNumber, address, blockNumber, label, opcode, operand, comment); // Read next line
...
int main()
{
  ...
  {
    writestring += it.first + ":-\t" + "name:" + it.second.name + "\t|" + "address:" + it.second.address + "\t|" + "relative:" + inttostrhe(it.second.relative) + " \n";
  }
