...
string stringget(char c)
{
  string s(1, c);
  return s;
}

string inttostrhe(int x, int fill = 5)
{
  stringstream s;
...
string stringexpand(string data, int length, char fillChar, bool back = false)
{
  if (back)
  {
...
string str_to_he_str(const string &input)
{
...
void readFirstNonWhiteSpace(string line, int &index, bool &status, string &data, bool readTillEnd = false)
{
  ...
  while (index < line.length() && !ch_wh_sp(line[index]))
  {
    data += line[index];
    index++;
  }
